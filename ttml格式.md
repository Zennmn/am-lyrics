...
2. **x-bg 内括号带时间戳 span → 部分设备出现"整组全亮→全灭→逐字高亮"闪 bug**（独立的 `( )` token 成了多余高亮单元，卡在 AM 组/字高亮模式切换点；部分设备/版本实测可正常渲染，校验器仅警告）
3. **x-bg 内括号做无时间戳纯文本也不可靠**：半角 `( )` 被 AM 吞掉不显示，全角 `（）` 开头被吞、句尾漏出孤零零 `）`
4. **结论：x-bg 内括号尽量不输出**（x-bg 靠浅色区分，不需要括号标记）；如需保留括号并入词 span 写法，以实际设备渲染为准，校验器仅警告
5. ME! 官方参考的写法是括号**并入词 span**（`(Ye` / `ah)`）——可行但不逐字
6. AMLL 规范建议 x-bg 用**半角括号包裹**、最多一对括号、背景声先于主声时 span 放主声前否则行尾——但 AM 括号显示可靠性因设备/版本而异（见上），校验器仅警告
...
- **纯文本，零改写，零 span**（实测部分设备/版本可正常渲染 text 内 span，校验器仅警告；保守起见优先纯文本）
- 无翻译的哼唱/拟声行：**原词回填**作为 `<text>` 内容（回填同步剥引号/不成对括号）

**实测雷区：**
1. `<text>` 内塞 x-bg span → 部分设备**整条翻译轨失效**（部分设备/版本实测可正常渲染，校验器仅警告；保守起见优先合并为纯文本并加全角括号）
